...
import com.android.model.wrapper.UserDataResponse;
...
public class MainActivity extends BaseAppCompatActivity implements View.OnClickListener,
        ...
        UserDataAdapter.OnDropdownClickListener {
    private final String TAG = getClass().getSimpleName();
    private Activity mActivity = MainActivity.this;
    private ActivityMainBinding layoutBinding;

    private UserDataAdapter.OnDropdownClickListener onDropdownClickListener;
    private UserDataAdapter adapter_feedback;
    private ArrayList<UserDataModel> list_feedback;
    ...
    private void initListener() {
        layoutBinding.etSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence charSequence, int start, int before, int count) {
                if (adapter_feedback != null) {
                    adapter_feedback.getFilter().filter(charSequence.toString());
                }
            }
    ...
    /**
     * fill feedback list into adapter
     */
    private void fillAdapter_feedback() {
        if (list_feedback != null && list_feedback.size() > 0) {
            layoutBinding.rvData.setVisibility(View.VISIBLE);
            layoutBinding.llNoData.setVisibility(View.GONE);

            if (adapter_feedback == null) {
                adapter_feedback = new UserDataAdapter(mActivity, onDropdownClickListener);
                adapter_feedback.setOnItemClickListener(this);
            }
            adapter_feedback.doRefresh(list_feedback);
            ...
            if (layoutBinding.rvData.getAdapter() == null) {
                ...
                layoutBinding.rvData.setAdapter(adapter_feedback);
            }
        } else {
    ...
    public void request_data() {
        ...
        Call call = APIClient.appInterface(false).get(
                APIClient.RQ_DATA);

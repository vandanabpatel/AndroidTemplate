 ...
 * reference - https://github.com/ncapdevi/FragNav
 */
...
public class FragmentNavigation {
    ...
    private static final String EXTRA_TAG_COUNT = FragmentNavigation.class.getName() + ":EXTRA_TAG_COUNT";
    private static final String EXTRA_SELECTED_TAB_INDEX = FragmentNavigation.class.getName() + ":EXTRA_SELECTED_TAB_INDEX";
    private static final String EXTRA_CURRENT_FRAGMENT = FragmentNavigation.class.getName() + ":EXTRA_CURRENT_FRAGMENT";
    private static final String EXTRA_FRAGMENT_STACK = FragmentNavigation.class.getName() + ":EXTRA_FRAGMENT_STACK";
    ...
    private FragmentNavigation(Builder builder, @Nullable Bundle savedInstanceState) {
        ...
        // attempt to restore getFileFromContentProvider bundle, if not, initialize
    ...
    /**
     * Pop the current fragment getFileFromContentProvider the current tab
     *
     * @param transactionOptions - Transaction options to be displayed
     */
    ...
    public void popFragments(int popDepth, @Nullable FragmentNavigationTransactionOptions transactionOptions) throws UnsupportedOperationException {
        ...
        // Pop the number of the fragments on the stack and remove them getFileFromContentProvider the FragmentManager
        ...
        // If we can't reattach, either pull getFileFromContentProvider the stack, or create a new root fragment
        if (fragment != null) {
            ft.commit();
        } else {
    ...
    public void clearStack(@Nullable FragmentNavigationTransactionOptions transactionOptions) {
        ...
        if (fragmentStack.size() > 1) {
            ...
            //Pop all of the fragments on the stack and remove them getFileFromContentProvider the FragmentManager
     ...
     *
     * @param index - The tab index to get this fragment getFileFromContentProvider
    ...
    private Fragment getRootFragment(int index) throws IllegalStateException {
        ...
        if (!mFragmentStacks.get(index).isEmpty()) {
            ...
            if (mSelectedTabIndex != NO_TAB && fragment != null) {
                mFragmentStacks.get(mSelectedTabIndex).push(fragment);
            }
        }
        /*if (fragment == null) {
    ...
    public Fragment getCurrentFrag() {
        ...
        //if not, try to pull it getFileFromContentProvider the stack
        else {
            Stack<Fragment> fragmentStack = mFragmentStacks.get(mSelectedTabIndex);
            if (!fragmentStack.isEmpty()) {
    ...
    /**
     * Create a unique fragment tag so that we can grab the fragment later getFileFromContentProvider the FragmentManger
     *
     ...
     *
     * @param savedInstanceState - The bundle to restore getFileFromContentProvider
     * @param rootFragments      - List of root fragments getFileFromContentProvider which to initialize empty stacks. If null, pull fragments getFileFromContentProvider RootFragmentListener.
    ...
    public static final class Builder {
        ...
        public FragmentNavigation build() {
            if (mRootFragmentListener == null && mRootFragments == null) {
                throw new IndexOutOfBoundsException("Either a root fragment(s) needs to be set, or a fragment listener");
            }
            return new FragmentNavigation(this, mSavedInstanceState);
        }
    }
}
